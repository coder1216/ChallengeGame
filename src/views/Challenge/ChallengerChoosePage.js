...
export default class ChooseChallenger extends Component {
    ...
    return fetch(url,{
      ...
      .then((responseJson) => {
        ...
        if(this.state.data[0].username === this.state.whoToPair.username){
          ...
          this.sendNotification(this.state.data[0].username);
        }else{
  ...
  sendNotification(username){
    const url = `http://104.236.189.217:8888/sendNotification`;
    this.setState({ loading: true });
    ...
    return fetch(url,{
      ...
      body: JSON.stringify({
        username: [username],
        description:"You have been paired with " + this.props.myNickname + ". Go to complete the challenge!",
      }),
    })
      .then((response) => response.json())
      .then((responseJson) => {
    ...
    clickBack() {
        this.setState({showGo: false,whoBeClick:{username:'',nickname:''},clicked:false});
